...
# TODO: USE CROSS TABULATION TO CHECK
...
# Arrange by date_of_diagnosis, patient_number.
cases_8003 <-  cases_8003 %>% arrange(patient_number, date_of_diagnosis)
...
cases_8003 <- cases_8003 %>%
  mutate(age_gp = cut(age_diagnosed, breaks = c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 85, Inf),
                      labels = FALSE,
                      right = FALSE)) 
...
cases_8003 <- cases_8003 %>% group_by(year, age_gp) %>% summarise(cases=n())

# Combine cases from 80-17.
cases_combined <- base::rbind(cases_8003, cases_0417)
