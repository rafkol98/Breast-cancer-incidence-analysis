...
# If a patient has only one row (not duplicated entry) and the tumour number is 2 -> remove this as it means that patient is prevalence
# in the cases before 1980.
# remove_duplicates <- function(df) {
#   # Iterate through patients id.
#   for (number in df$patient_number) {
#     # Get all the rows of the current patient.
#     patient_rows <- filter(df, patient_number==number)
#     
#     # If there is exactly one row of the patient and the tumour number is greater than 1,
#     # then it means that patient had their first tumour before 1980 -> prevalence -> remove them.
#     if (nrow(patient_rows) == 1 && patient_rows$tumour_number > 1) {
#       df <- subset(df, patient_number != number)
#     }
#       
#   }
# }
...
european_standard <- read_excel("./Data/european-standard-pop.xlsx") %>% rename(age = ageband)
...
cases_8003 <- read.csv("./Data/tumour.tsv", sep = '\t') %>% mutate(date_of_diagnosis = dmy(date_of_diagnosis))
...
cases_8003 <- cases_8003 %>% group_by(patient_number) %>% summarise(date_of_diagnosis = min(date_of_diagnosis))
...
cases_8003 <- merge(patient, cases_8003, by="patient_number") %>% mutate(age_diagnosed = as.integer(interval(date_of_birth, date_of_diagnosis) / years(1)))

# Get age group of patient.
cases_8003 <- cases_8003 %>%
  mutate(age = cut(age_diagnosed, breaks = c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 85, Inf),
                      labels = FALSE,
                      right = FALSE)) 
...
cases_8003 <- cases_8003 %>% group_by(year, age) %>% summarise(cases=n())

# Combine cases from 80-17.
cases_combined <- base::rbind(cases_8003, cases_0417) %>% arrange(year, age)

#### YEARLY INCIDENCE RATE FROM 1980 TO 2017 ####
# Combine cases and population
combined_cases_population <- left_join(cases_combined, pop_8017, by=c("year","age"))
combined_cases_population <- combined_cases_population %>% mutate(crude_rate = (cases/population) * 100000)

# Calculate age distribution proportions on standard population.
european_standard <- european_standard %>% mutate(age_distribution_proportions = europop/sum(europop))

combined_cases_population <- left_join(combined_cases_population, european_standard, by="age") 

combined_cases_population <- combined_cases_population %>% mutate(expected_incidence = crude_rate*age_distribution_proportions)

direct_age_standardised_death_rate <- combined_cases_population %>% group_by(year) %>% summarise(DASDR = sum(expected_incidence))
